  ...
  $.login = function(text, title, onOK, onCancel, username, password) {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p>' +
            '<input type="text" class="weui-input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || '') + '" placeholder="输入用户�?" />' +
...
+ function($) {
  ...
  $.showLoading = function(text) {
    ...
    html += '</div>';
    html += '<p class="weui-toast_content">' + (text || "数据加载�?") + '</p>';
...
+ function($) {
  ...
  Select.prototype.open = function(values, titles) {
    ...
    // open picker 会默认关掉其他的，但�? onClose 不会被调用，所以这里先关掉其他select
    ...
    if(!force) {
      ...
      if(this.config.multi) {
        if(this.config.min !== undefined && this.data.length < this.config.min) {
          $.toast("请至少选择"+this.config.min+"�?", "text");
          return false
        }
        if(this.config.max !== undefined && this.data.length > this.config.max) {
          $.toast("最多只能选择"+this.config.max+"�?", "text");
          return false
        }
      }
    }
  ...
  defaults = $.fn.select.prototype.defaults = {
    items: [],
    input: undefined, //输入框的初始�?
    title: "请选择",
    multi: false,
    closeText: "确定",
    autoClose: true, //是否选择完成后自动关闭，只有单选模式下才有�?
    onChange: undefined, //function
    beforeClose: undefined, // function 关闭之前，如果返回false则阻止关�?
    onClose: undefined, //function
    onOpen: undefined, //function
    split: ",",  //多选模式下的分隔符
    min: undefined, //多选模式下可用，最少选择�?
    max: undefined, //单选模式下可用，最多选择�?
    toolbarTemplate: '<div class="toolbar">\
      <div class="toolbar-inner">\
...
+function ($) {
  ...
  var Calendar = function (params) {
      ...
      //iphone 上无法正确触�? click，会导致点击外面无法关闭
...
+ function($) {
  ...
  defaults = $.fn.datetimePicker.prototype.defaults = {
    ...
    parse: function (str) {
      // 把字符串转换成数组，用来解析初始�?
      // 如果你的定制的初始值格式无法被这个默认函数解析，请自定义这个函数。比如你的时间是 '子时' 那么默认情况这个'�?'会被当做分隔符而导致错误，所以你需要自己定义parse函数
      // 默认兼容的分隔符
      var t = str.split(this.datetimeSplit);
      return t[0].split(/\D/).concat(t[1].split(/:|时|分|�?/)).filter(function (d) {
        return !!d;
      })
    }
  }
...
+ function($) {
  ...
  //Popup �? picker 之类的不要共用一个弹出方法，因为这样会导�? �? popup 中再弹出 picker 的时候会有问题�?
...
+function ($) {
  ...
  Swipeout.prototype.touchStart = function(e) {
    ...
    this.diffX = this.diffY = 0;
    this._closeOthers()
    this.limit = this.container.find('>.weui-cell__ft').width() || 68; // 因为有的时候初始化的时候元素是隐藏的（比如在对话框内），所以在touchstart的时候计算宽度而不是初始化的时�?
  };
  ...
  Swipeout.prototype.touchEnd = function() {
    ...
    if (this.diffX < -5 && t < 200) { // 向左快速滑动，则打开
      this.open()
    } else if (this.diffX >= 0 && t < 200) { // 向右快速滑动，或者单�?,则关�?
      this.close()
